...
export default function ReleaseNotes({ accent = '#1d9e75' }) {
  ...
  // The panel is rendered into document.body via a portal so it escapes the
  // sticky header's stacking context — otherwise the bottom tab nav paints over
  // it and iOS Safari mis-handles the fixed positioning (vanishing text, the
  // background hijacking scroll).
  const panel = open ? createPortal(
    <div onClick={() => setOpen(false)} role="dialog" aria-modal="true" aria-label="What's new"
      style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
