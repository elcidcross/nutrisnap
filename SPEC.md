...
- Score per item = % of the weekly target met, capped at 100%, mapped to a US letter scale (`letterFor`: A+ ≥97 … D- ≥60, F <60). Overall = the **GPA** of the subject grades (each letter → grade points, averaged via `letterFromPoints`) **rounded down** to its band — so a single weak subject can't collapse the card to F, and an A+ overall requires every subject to be A+.
...
**UI.** A swipeable, manila-folder "certificate" per finished week — labelled by ISO year + week (`weekLabel` → "2026 Week 24") with the date range beneath — in **chronological** order (newest last; opens on it, swipe back for older). The in-progress week is hidden until it ends. Below the carousel: a persona row, page dots, and an at-a-glance **History** list (tap a row to page the carousel to that week and scroll it into view). Subjects expand to a plain-language "why" (nutrition: per-macro daily avg vs target → "too much fat, not enough protein"; habits: this week vs weekly target). N/A subjects are hidden. A native **Share** button appears where the Web Share API is available (HTTPS/localhost only). Read-only, and **re-fetches on every activation** (it aggregates other apps' data, which may change mid-session).

**AI teacher's note.** The old static caption is replaced by an **AI-generated note** (`getReportCardNote` in `src/utils/api.js`, through the `/api/claude` proxy — same path as `getNudge`). It reads the week's grades + per-macro/target detail, the multi-week trend, and the user's active deadline **Goals** (current value + recent body readings, so it can do the pace math toward e.g. "16% body fat by Jul 14"). Three switchable **personas** set the tone — `analytical` (default), `encouraging`, `tough` — in ~3 sentences. The prompt context is assembled by the pure, unit-tested `buildNoteContext` (`reportcard.js`). Notes generate **lazily** for the card in view, are **saved** per (week, persona) in `report_card_notes` (migration `sql/report_card_notes.sql`), with a **Refresh** (↻) to regenerate; persona persists in `localStorage` (`nutrisnap_reportcard_persona`).
